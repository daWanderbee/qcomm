#!/usr/bin/env node
// Prints a chat-friendly summary of the latest BigBasket scan (data/rank.csv + data/competitors.csv):
...
function parseCSV(t){
  ...
  const h=rows.shift(); return rows.filter(r=>r.some(x=>x!=='')).map(r=>Object.fromEntries(h.map((k,j)=>[k,(r[j]||'').trim()])));
}
const read = f => fs.existsSync(f) ? parseCSV(fs.readFileSync(f,'utf8')) : [];

const rank=read('data/rank.csv'), comp=read('data/competitors.csv');
if(!rank.length && !comp.length){ console.log('No scan data yet. Run tools/collect-bigbasket.mjs first.'); process.exit(0); }
const seeds=fs.existsSync('data/seeds.txt')
  ? fs.readFileSync('data/seeds.txt','utf8').split('\n').map(s=>s.trim()).filter(Boolean)
  : [...new Set(comp.map(r=>r.keyword))];

const byKw={}; comp.forEach(r=>{(byKw[r.keyword]=byKw[r.keyword]||[]).push(r);});
Object.values(byKw).forEach(a=>a.sort((x,y)=>(+x.rank||99)-(+y.rank||99)));
const myBest={}; rank.forEach(r=>{const v=+r.rank; if(!myBest[r.keyword]||v<myBest[r.keyword])myBest[r.keyword]=v;});
...
const bc={}; comp.forEach(r=>{bc[r.competitor]=(bc[r.competitor]||0)+1;});
console.log('TOP RIVALS: '+Object.entries(bc).sort((a,b)=>b[1]-a[1]).slice(0,6).map(([b,n])=>`${b}(${n})`).join('  '));
